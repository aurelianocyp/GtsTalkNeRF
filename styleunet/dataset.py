...
class TorsoDataset(Dataset):
    def __init__(self, path, transform_inp, transform_out):
        ...
        inp_list = sorted(os.listdir(os.path.join(path, 'log', 'stage2', 'results')))
        self.inp_list = [os.path.join(path, 'log', 'stage2', 'results', imgpath) for imgpath in inp_list if 'rgb' in imgpath]
